...
ct.inputs.addAction('Pause', [{"code":"keyboard.Space"}]);
ct.inputs.addAction('MoveY', [{"code":"keyboard.KeyW","multiplier":-1},{"code":"keyboard.KeyS","multiplier":1},{"code":"keyboard.ArrowUp","multiplier":-1},{"code":"keyboard.ArrowDown","multiplier":1}]);
...
ct.rooms.templates['NeonPower'] = {
    name: 'NeonPower',
    width: 1278,
    height: 1022,
    /* JSON.parse allows for a much faster loading of big objects */
    objects: JSON.parse('[{"x":704,"y":896,"exts":{},"type":"Player"}]'),
    bgs: JSON.parse('[{"depth":-5,"texture":"background","extends":{"parallaxX":0.25,"repeat":"repeat","movementY":6,"parallaxY":0.25}}]'),
    tiles: JSON.parse('[{"depth":-5,"tiles":[],"extends":{}}]'),
    backgroundColor: '#000000',
    onStep() {
        this.policeTimer -= ct.delta;
if (this.policeTimer <= 0) {
    this.policeTimer = 75;
    ct.types.copy('Policecar',ct.random.dice(448, 576, 704, 832));
    ct.types.copy('Policecar',ct.random.dice(448, 576, 704, 832));
}

this.powerupTimer -= ct.delta;
if (this.powerupTimer <= 0) {
    this.powerupTimer = 1000;
    ct.types.copy('PowerUp',ct.random.dice(448, 576, 704, 832));
}
...
(function resAddon(ct) {
    ...
    ct.res = {
        ...
        soundsError: 0,
        sounds: {},
        registry: [{"power_up":{"frames":1,"shape":{"type":"rect","top":16,"bottom":16,"left":16,"right":16},"anchor":{"x":0.5,"y":0.5}},"police-car":{"frames":1,"shape":{"type":"strip","points":[{"x":0,"y":-95},{"x":30.25,"y":-92},{"x":31.75,"y":86},{"x":0.5,"y":96.75},{"x":-31.25,"y":87.25},{"x":-30.25,"y":-93.25}],"closedStrip":true},"anchor":{"x":0.5,"y":0.49740932642487046}},"protocar":{"frames":1,"shape":{"type":"strip","points":[{"x":1,"y":-88.5},{"x":23.75,"y":-80},{"x":37.25,"y":-58.25},{"x":36.25,"y":-13.75},{"x":38,"y":76.25},{"x":19.75,"y":89.5},{"x":1.5,"y":89},{"x":-17.25,"y":89.25},{"x":-31.5,"y":83.75},{"x":-37.75,"y":71.5},{"x":-36,"y":-59.25},{"x":-28.25,"y":-74.75}],"closedStrip":true},"anchor":{"x":0.4942528735632184,"y":0.5}},"background":{"atlas":"./img/t0.png","frames":0,"shape":{"type":"rect","top":512,"bottom":512,"left":639,"right":639},"anchor":{"x":0.5,"y":0.5}},"straßenrand":{"frames":1,"shape":{"type":"rect","top":0,"bottom":1024,"left":0,"right":384},"anchor":{"x":0,"y":0}},"straße":{"frames":1,"shape":{"type":"rect","top":0,"bottom":1024,"left":0,"right":512},"anchor":{"x":0,"y":0}},"police-car2":{"frames":1,"shape":{"type":"strip","points":[{"x":73,"y":-80},{"x":107,"y":-21},{"x":-69.75,"y":79.5},{"x":-98,"y":57.75},{"x":-104,"y":24}],"closedStrip":true},"anchor":{"x":0.4976303317535545,"y":0.5}}}][0],
...
ct.types.templates["Player"] = {
    ...
    onStep: function () {
        //Bewegt das Auto
this.x += 10 * ct.delta * ct.actions.MoveX.value; // Move by X axis
//this.y += 10 * ct.delta * ct.actions.MoveY.value; // Move by Y axis
//Checkt ob links oder recht eine Border ist
if (this.x < 448) {
    this.x = 448;
}
if (this.x > 832) {
...
if (collided) {
    ...
    onCreate: function () {
        this.speed = 15;
this.direction = 270;
//this.rotation = ct.random.dice(60, -60, 0);

    },
    extends: {}
};
ct.types.list['Policecar'] = [];
...
ct.types.templates["police-car2"] = {
    depth: 0,
    texture: "police-car2",
    onStep: function () {
        this.move();
...
ct.sound.init('carcrash', {
    wav: './snd/f9cd5126-467d-4138-b655-bd078a21b5b7.wav',
    mp3: false,
    ogg: false
}, {
    poolSize: 3,
